...
export async function playRoom({ BASE, room, bot, summon, log }) {
  ...
  const roomCtl = sse(`${BASE}/room/events?room=${room}`, (entry) => {
    ...
    // only FRESH hellos name the host (waiting hosts heartbeat every 45s);
    // stale hellos in the replay belong to dead sessions
    if (m.type === 'hello' && !started && Date.now() - (entry.t || 0) < 90000) { hostPid = m.from; return; }
  ...
  try {
    ...
    if (!hostPid) await new Promise((r) => setTimeout(r, 4000));
    if (!hostPid) throw new Error('no live host in room');
