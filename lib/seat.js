...
export async function playRoom({ BASE, room, bot, summon, summonT = 0, log }) {
  ...
  try {
    ...
    for (;;) {
      try {
      // any FRESH start from our host names the hand — the host's loop may
      // be far past 1 if a previous bot died mid-session; we adopt its count
      const start = await next((m) => m.type === 'start' && m.from === hostPid
        && m.handNo !== lastHand && Date.now() - (m._t || 0) < 180000);
