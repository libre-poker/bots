...
export function makeClient(BASE) {
  // sends retry through blips like the event stream reconnects through
  // them — a dropped POST delays a hand, never kills a seat
  const post = async (path, body, token) => {
    ...
    for (let a = 0; ; a++) {
      try {
        ...
        await new Promise((r2) => setTimeout(r2, delays[a]));
      }
    }
  };
  function sse(url, onEvent, onClose) {
    // a dropped stream reconnects itself until aborted; the caller's
    // handlers must be (and are) idempotent under replay.
    // The server pings every 25s, so a minute of total silence means the
    // TCP stream is half-dead (no error will ever come) — cancel and
    // reconnect rather than stay deaf on a watched room forever.
    const IDLE = 60000;
        ...
        try {
          ...
          for (;;) {
            ...
            const got = await Promise.race([
              reader.read(),
              new Promise((r) => { tid = setTimeout(() => r('idle'), IDLE); }),
            ]);
            clearTimeout(tid);
            if (got === 'idle') { try { await reader.cancel(); } catch { /* gone */ } break; }
...
export async function playRoom({ BASE, room, bot, summon, summonT = 0, log }) {
  ...
  const inbox = [], waiters = [];
  const send = (msg) => {
    const body = { from: pid, seq: seq++, agent: bot.agent, ...msg };
    // a declared bot signs what it says: the attestation rides along
    if (bot.attest) { try { body.attest = bot.attest({ room, ...body }); } catch { /* unsigned beats silent */ } }
    return post('/room/send', { room, msg: body });
  };
  ...
  let helloT = Date.now();
  const die = (why) => {
    for (const w of waiters.splice(0)) w.reject?.(new Error(why));
  };
  const seen = new Set();
  const roomCtl = sse(`${BASE}/room/events?room=${room}`, (entry) => {
    ...
    // our own hello echoed back carries the SERVER's clock — adopt it so
    // the newer-than-hello gate on starts never trips on clock skew
    if (m.from === pid) { if (m.type === 'hello' && entry.t) helloT = entry.t; return; }
    // a fresh summon from a NEW host means our seating is stale — stand up
    // so the watcher can re-seat us opposite the live summoner
    if (m.type === 'summon' && Date.now() - (entry.t || 0) < 60000
      && (entry.t || 0) > summonT + 1000
      && (m.bot ? m.bot === bot.id : bot.id === 'ladder')) {
      if (m.from === hostPid) {
        // our own host summoning again: their page restarted and lost our
        // hello — abandon whatever hand we were in and greet them anew
        rehello = true;
        return die('host re-summoned');
      }
      return die('reseated: new summoner ' + m.from.slice(0, 8));
    }
  ...
  try {
    ...
    helloT = Date.now();
    await send({ type: 'hello', name, re: hostPid, level });
    ...
    for (;;) {
      try {
      // only a start NEWER than our own hello can bind us to a deck — the
      // room replays every start ever sent, and a stale one whose session
      // we can still claim would seal this seat to the wrong shuffle
      const start = await next((m) => m.type === 'start' && m.from === hostPid
        && m.handNo !== lastHand && (m._t || 0) > helloT, 60 * 60000);
      const handNo = start.handNo;
      lastHand = handNo;
      ...
      cCtl = sse(`${BASE}/events?sid=${start.sid}&token=${claimed.token}`, (ev) => {
      ...
      while (h.phase === 'act' && guard++ < 200) {
        const L = legal(h);
        if (L.seat === 1) {
          await new Promise((r) => setTimeout(r, summon.turbo ? 40 : 600 + rng() * 900));
          const a = await bot.decide({ h, seat: 1, L, ...ctx });
          act(h, a);
          await send({ type: 'act', handNo, sid: start.sid, action: a.action, amount: a.amount ?? null });
        } else {
          const m = await next((x) => x.type === 'act' && x.sid === start.sid && x.from === hostPid);
          const L2 = legal(h);
      ...
      cCtl.abort();
        ...
        if (!rehello) throw e;
        rehello = false; lastHand = -1;
        cCtl?.abort();
        helloT = Date.now();
        await send({ type: 'hello', name, re: hostPid, level });
        log(`re-helloed ${hostPid.slice(0, 8)} in ${room} — their page restarted`);
      }
    }
  } catch (e) {
...
export function runFleet({ BASE, bot, max = 4, watchRoom = null, log = console.log }) {
  ...
  const engaged = new Map(), watched = new Map(), cooldown = new Map();
  ...
  function watch(room) {
    ...
    if ((cooldown.get(room) || 0) > Date.now()) return;
    const ctl = sse(`${BASE}/room/events?room=${room}`, (entry) => {
      ...
      if (matches(m, entry.t) && !engaged.has(room) && engaged.size < max) {
        ...
        playRoom({ BASE, room, bot, summon: m, summonT: entry.t || 0, log })
          .finally(() => { engaged.delete(room); cooldown.set(room, Date.now() + 20000); });
      }
    }, () => watched.delete(room));
